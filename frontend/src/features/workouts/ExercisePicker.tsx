          ...
          <div className="max-h-[52vh] overflow-y-auto pr-1">
            {groups.map((g) => (
              <section key={g.muscle} className="mb-4">
                <h3 className="px-0.5 text-[11px] tracking-[0.14em] text-ink-faint uppercase">{g.label}</h3>
                  ...
                  {g.exercises.map((e) => (
                    <li key={e.key}>
                      <button
                        type="button"
                        onClick={() => onPick(e)}
                        className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left active:bg-surface"
                      >
